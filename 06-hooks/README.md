...
钩子是一种自动化脚本，在 Claude Code 会话期间响应特定事件执行。它们支持自动化、验证、权限管理和自定义工作流。

## 概述

钩子是自动化操作（Shell 命令、HTTP Webhook、LLM 提示词或子代理评估），当 Claude Code 中发生特定事件时自动执行。它们接收 JSON 输入，通过退出码和 JSON 输出进行通信。
...
| `type` | 钩子类型：`"command"`（Bash）、`"prompt"`（LLM）、`"http"`（Webhooks）或 `"agent"`（子代理） | `"command"` |
| `command` | 要执行的 Shell 命令 | `"$CLAUDE_PROJECT_DIR/.claude/hooks/format.sh"` |
| `timeout` | 可选超时秒数（默认 60） | `30` |
| `once` | 如果为 `true`，钩子仅在会话中运行一次 | `true` |
...
远程 Webhook 端点，接收与命令钩子相同的 JSON 输入。HTTP 钩子向 URL POST JSON 并接收 JSON 响应。启用沙箱时，HTTP 钩子通过沙箱路由。URL 中的环境变量插值需要显式的 `allowedEnvVars` 列表以确保安全。
...
- `"url"` -- Webhook 端点 URL
- 启用沙箱时通过沙箱路由
- URL 中任何环境变量插值需要显式的 `allowedEnvVars` 列表
...
| **TeammateIdle** | 代理团队成员空闲时 | （无） | 是 | 团队成员协调 |
...
| **FileChanged** | 监视的文件更改时 | （无） | 否 | 文件监视、重建 |
...
| **ElicitationResult** | 用户响应征求时 | （无） | 是 | 响应处理 |
| **SessionEnd** | 会话终止时 | （无） | 否 | 清理、最终日志记录 |

### PreToolUse

在 Claude 创建工具参数后、处理前运行。使用此钩子验证或修改工具输入。
...
**输出控制：**
- `"block"` 决策提示 Claude 提供反馈
- `additionalContext`：为 Claude 添加的上下文

### UserPromptSubmit

当用户提交提示词时，在 Claude 处理之前运行。
...
当 Claude 完成响应（Stop）或子代理完成（SubagentStop）时运行。支持基于提示词的评估以进行智能任务完成检查。

**附加输入字段：** `Stop` 和 `SubagentStop` 钩子在 JSON 输入中接收 `last_assistant_message` 字段，包含 Claude 或子代理停止前的最后一条消息。这对于评估任务完成很有用。
...
当子代理开始执行时运行。匹配器输入是代理类型名称，允许钩子针对特定子代理类型。
...
这允许在与使用它们的组件中直接定义钩子，保持相关代码在一起。

### 子代理 Frontmatter 中的钩子

当在子代理的 Frontmatter 中定义 `Stop` 钩子时，它会自动转换为作用域为该子代理的 `SubagentStop` 钩子。这确保该停止钩子仅在该特定子代理完成时触发，而不是在主会话停止时触发。
...
使用 `UserPromptSubmit`（消息前钩子）和 `Stop`（消息后钩子）跟踪每个请求的令牌消耗。
...
def get_state_file(session_id: str) -> str:
    """Get temp file path for storing pre-message token count."""
    return os.path.join(tempfile.gettempdir(), f"claude-context-{session_id}.json")

def count_tokens(text: str) -> int:
    """Count tokens using character-based estimation: ~4 chars per token."""
    return len(text) // 4
...
def handle_stop(data: dict) -> None:
    ...
    print(f"Context: ~{current_tokens:,} tokens ({percentage:.1f}% used, ~{remaining:,} remaining)", file=sys.stderr)
    if delta_tokens > 0:
...
**工作原理：**
1. `UserPromptSubmit` 在处理提示词前触发 - 保存当前令牌计数
2. `Stop` 在 Claude 响应后触发 - 计算增量并报告使用情况
3. 每个会话通过 temp 文件名中的 `session_id` 隔离

### 示例 7：种子自动模式权限（一次性设置脚本）

一次性设置脚本，向 `~/.claude/settings.json` 添加约 67 条安全权限规则，等同于 Claude Code 自动模式基线。运行一次即可；可以安全地重新运行（跳过已存在的规则）。
...
- **需要工作区信任：** `statusLine` 和 `fileSuggestion` 钩子输出命令现在需要工作区信任接受后才能生效。
- **HTTP 钩子和环境变量：** HTTP 钩子需要显式的 `allowedEnvVars` 列表才能在 URL 中使用环境变量插值。这可以防止敏感环境变量意外泄露到远程端点。
- **托管设置层次结构：** `disableAllHooks` 设置现在遵循托管设置层次结构，这意味着组织级别的设置可以强制执行个别用户无法覆盖的钩子禁用。
...
| 首先单独测试钩子 | 部署未测试的钩子 |
| HTTP 钩子使用显式 `allowedEnvVars` | 向 Webhook 暴露所有环境变量 |
...
使用 `Ctrl+O` 在 Claude Code 中启用详细模式，查看钩子执行进度。
...
- 验证钩子从 stdin 读取 JSON（而不是命令参数）

### 钩子意外阻塞
- 使用示例 JSON 测试钩子：`echo '{"tool_name": "Write", ...}' | ./hook.py`
- 检查退出码：应为 0 表示允许，2 表示阻止
...
### 步骤 3：在设置中配置
编辑 `~/.claude/settings.json` 或 `.claude/settings.json`，添加如上所示的钩子配置。

## 相关概念

- **[Checkpoints and Rewind](../08-checkpoints/)** - 保存和恢复会话状态
- **[Slash Commands](../01-slash-commands/)** - 创建自定义斜杠命令
- **[Skills](../03-skills/)** - 可复用的自主能力
- **[Subagents](../04-subagents/)** - 委托任务执行
- **[Plugins](../07-plugins/)** - 捆绑扩展包
- **[Advanced Features](../09-advanced-features/)** - 探索 Claude Code 高级功能
